...
async function createDailyBackup(env: Env, scheduledTime: number) {
  ...
  const [users, jobs, entries, payWeeks, payRateHistory, timeOff, jobMismatchReviews, expenses, history] = await Promise.all([
    ...
    env.DB.prepare(`SELECT id, job_id AS jobId, created_by AS createdBy, purchase_date AS purchaseDate, vendor, category, amount, sales_tax AS salesTax, note, ocr_text AS ocrText, reviewed, receipt_key AS receiptKey, receipt_type AS receiptType, created_at AS createdAt, updated_at AS updatedAt FROM expenses ORDER BY purchaseDate, id`).all(),
